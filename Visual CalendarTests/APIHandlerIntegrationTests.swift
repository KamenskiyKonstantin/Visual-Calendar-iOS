...
class APIHandlerIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        _ = await api.signUp(email: email, password: password, confirmPassword: password)
        _ = await api.logout()
    }
    ...
    func testInvalidSignup() async throws {
        ...
        // 1. Duplicate signup
        let duplicateSignup = await api.signUp(email: email, password: password, confirmPassword: password)
        ...
        let weakPasswordSignup = await api.signUp(email: weakPasswordEmail, password: "123", confirmPassword: "123")
    ...
    func testReactionService() async throws {
        print("[-TESTER/APIHANDLER-] NOW TESTING: Reaction Service")

        // Step 1: Login
        let loginSuccess = await api.login(email: email, password: password)
        XCTAssertTrue(loginSuccess, "Login should succeed before reaction tests")
        XCTAssertTrue(api.isAuthenticated, "User should be authenticated after login")

        // Step 2: Create a `.once` event
        let eventOnce = makeMockEvent(color: "Red")
        let createdOnce = await api.createEvent(eventOnce)
        XCTAssertTrue(createdOnce, "Creating once event should succeed")

        // Step 3: Set a reaction
        let reactionOnce: EventReaction = .smiley
        let reactionSetOnce = await api.setReaction(for: eventOnce.id, timeStart: eventOnce.dateTimeStart.toIntList(), reaction: reactionOnce)
        XCTAssertTrue(reactionSetOnce, "Setting reaction on .once event should succeed")
        
        
        let reactionOnceObject = EventReactionRow(eventID: eventOnce.id, timestamp: eventOnce.dateTimeStart.toIntList(),
                                                  reaction: reactionOnce.rawValue)
        
        // Step 4: Fetch and verify reaction
        let reactions = await api.fetchAllReactions(for: [eventOnce])
        guard let fetchedOnceReaction = reactions[eventOnce.id] else {
            XCTFail("Reaction not found for once event")
            return
        }
        XCTAssertTrue(fetchedOnceReaction.contains(where: {$0 == reactionOnceObject}), "Expected reaction \(reactionOnce), got \(fetchedOnceReaction)")

        // Step 5: Update reaction
        let newReaction: EventReaction = .thumbsUp
        let reactionUpdate = await api.setReaction(for: eventOnce.id,timeStart: eventOnce.dateTimeStart.toIntList(), reaction: newReaction)
        XCTAssertTrue(reactionUpdate, "Updating reaction should succeed")
        
        let newReactionObject = EventReactionRow(eventID: eventOnce.id, timestamp: eventOnce.dateTimeStart.toIntList(), reaction: newReaction.rawValue)
        let updatedReactions = await api.fetchAllReactions(for: [eventOnce])
        XCTAssertTrue(updatedReactions[eventOnce.id]!.contains(where: {$0 == newReactionObject}), "Reaction should be updated to \(newReaction), was ")

        // Step 6: Test with `.weekly` event
        var eventWeekly = makeMockEvent(color: "Blue")
        eventWeekly.repetitionType = .weekly
        let createdWeekly = await api.createEvent(eventWeekly)
        XCTAssertTrue(createdWeekly, "Creating weekly event should succeed")

        let reactionWeekly: EventReaction = .upset
        let reactionSetWeekly = await api.setReaction(for: eventWeekly.id,timeStart: eventWeekly.dateTimeStart.toIntList(), reaction: reactionWeekly)
        XCTAssertTrue(reactionSetWeekly, "Setting reaction on weekly event should succeed")

        let finalReactions = await api.fetchAllReactions(for: [eventOnce, eventWeekly])
        let reactionWeeklyObject = EventReactionRow(eventID: eventWeekly.id, timestamp: eventWeekly.dateTimeStart.toIntList(), reaction: reactionWeekly.rawValue        )
        XCTAssertTrue(finalReactions[eventWeekly.id]!.contains(where: {$0 == reactionWeeklyObject}), "Reaction should be set to \(reactionWeekly) for weekly event")

        print("[-TESTER/APIHANDLER-] TESTING SUCCESSFUL: Reaction Service")
    }
